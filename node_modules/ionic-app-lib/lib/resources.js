  ...
  function validateSourceImages() {
    ...
    _.each(resTypePlatforms, function(resTypePlatform) {
      for (var x = 0; x < validSourceFiles.length; x++) {
        globalSourceFile = path.join(appDirectory, Settings.resourceDir, validSourceFiles[x]);
        platformSourceFile = path.join(appDirectory, Settings.resourceDir, resTypePlatform.platform, validSourceFiles[x]);
  ...
  return promise
    ...
    .then(function(data) {
      if(!data.widget.platform || (data.widget.platform.length == 1 && data.widget.platform[0]['$'].name != platform)) {
        logging.logger.info('Adding icons for platform:'.blue.bold, platform.green)
        addPlatformIcons(platform, data)
        addSplashScreenPreferences(data)
        writeConfigData(appDirectory, data)
      }
    });
}
...
function hasExistingResources(appDirectory) {
  ...
  var hasResources = fs.existsSync(path.join(appDirectory, 'resources')),
      hasAndroidResources = fs.existsSync(path.join(appDirectory, 'resources', 'android')),
      hasIosResources = fs.existsSync(path.join(appDirectory, 'resources', 'ios')),
      hasIconResource = fs.existsSync(path.join(appDirectory, 'resources', 'icon.png')),
      hasSplashResource = fs.existsSync(path.join(appDirectory, 'resources', 'splash.png'));
...
function copyIconFilesIntoResources(appDirectory, forceAddResources) {
  ...
  var ionicResourcesUrl = 'https://github.com/ElandFormular/eland-default-resources/archive/master.zip';
  logging.logger.debug('uzip to: ', unzipPath)
  logging.logger.info('Downloading Default Eland Resources'.yellow);
  return Utils.fetchArchive(unzipPath, ionicResourcesUrl)
    .then(function() {
      shelljs.config.silent = true
      shelljs.mv('resources/eland-default-resources-master/*', 'resources')
      shelljs.rm('-rf', 'resources/eland-default-resources-master')
      logging.logger.info('Done adding default Eland resources'.yellow)
    })
}
