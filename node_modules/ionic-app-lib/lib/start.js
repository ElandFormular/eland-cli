var fs = require('fs'),
    ...
    argv = require('optimist').boolean(['no-cordova', 'sass', 'list', 'eland']).argv,
...
var ELAND_WRAPPER_REPO_NAME = 'eland-app-base';
...
var DEFAULT_ELAND_APP = {
  "plugins": [
    "https://github.com/ElandFormular/eland-plugins-mdm.git",
    "https://github.com/ElandFormular/eland-plugin-launcApp.git",
...
Start.fetchWrapper = function fetchWrapper(options) {
  ...
  var elandRepoUrl = 'https://github.com/ElandFormular/' + ELAND_WRAPPER_REPO_NAME + '/archive/master.zip';

  if(typeof options.eland != 'undefined'){
    if(options.eland) {
      Utils.fetchArchive(options.targetPath, elandRepoUrl)
      .then(function() {
        var repoFolderName = ELAND_WRAPPER_REPO_NAME + '-master';
...
Start.loadAppSetup = function loadAppSetup(options) {
  var appSetup;
  if(typeof options.eland != null) {
    if(options.eland) {
      appSetup = DEFAULT_ELAND_APP;
    } else {
      appSetup = DEFAULT_APP;
    }
  } else {
    appSetup = DEFAULT_APP;
  }
  ...
  if(fs.existsSync(appJsonPath)) {
    try {
      ...
      logging.logger.info('appSetup : ' + appSetup);
      rm('-rf', appJsonPath);
    } catch(e) {
...
Start.fetchIonicStarter = function(options) {
  ...
  if(typeof options.eland != 'undefined') {
    if(options.eland) {
      ...
      var repoName = ['eland-starter-', options.template].join('')
      var repoUrl = ['https://github.com/ElandFormular/', repoName].join('')
  ...
  try {
    ...
    parseString(configString, function (err, jsonConfig) {
      ...
      if(!packageName) {
        ...
        // Change PackageName(com.eland.[ProjectName])
        packageName = 'com.eland.' + directoryName.replace(/\./g, '');
        iosPackage = 'com.eland.ios.' + directoryName.replace(/\./g, '').replace(/ /g, '').replace(/-/g, '').replace(/_/g, '').toUpperCase().trim();
      }
      ...
      configString = configString.replace('ios-CFBundleIdentifier=\"com.eland.ios.starter\"', 'ios-CFBundleIdentifier=\"' + iosPackage + '\"');
